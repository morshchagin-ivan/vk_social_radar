...
Baseline 1.0 + U02/U05/U09 · 2026-09-23. Working MVP → Architecture Audit → Known Gaps → ADR → Target Architecture → Prioritized Upgrade Backlog → Controlled Evolution. После documentary baseline реализованы migration, provider boundary и generation resilience; остальные milestones сохраняют собственные acceptance gates.
...
flowchart LR
  ...
  Migration --> Data["PLANNED: Source Integrity - U03"]
  Data --> Boundaries["PLANNED: API Contract - U04"]
...
[Backlog U01–U17](05_UPGRADE_BACKLOG.md) остаётся исходным приоритетным списком. P2: U14 scheduling/durable jobs, U15 local export/restore, U16 graph/scores, U17 search/retrieval evolution. U02/U05/U09 закрыты в заявленном scope; U03 — следующий рекомендуемый implementation increment, без автоматического начала. RAG NO_RAG/PLANNED, Ollama/fallback NO. [Baseline report](BASELINE_UPGRADE_REPORT.md) остаётся историческим; новые результаты находятся в отдельных build reports.
