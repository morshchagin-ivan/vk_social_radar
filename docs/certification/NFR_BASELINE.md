...
| Reproducibility: canonical command | U08: 222 tests, seven gates, 18 invariants PASS | retain one command; remote U08 not run | Python 3.13.2/Node 22.14.0 on Windows; [reference](QUALITY_GATE_REFERENCE.md) | U07/U08 local IMPLEMENTED |
...
U07 quality duration is a measured development-loop observation, not an API/collector/inference SLA or multi-platform benchmark. All mandatory test evidence now comes from one runner; no separate legacy-function count remains. Mermaid formal rendering and live E2E are not automated evidence.

## U08 measured retrieval baseline

[Evaluation](RAG_EVALUATION.md): 18 synthetic cases, Recall@3=1.0, MRR=1.0 and no-evidence accuracy=1.0; enforced minima 0.90/0.90/1.00. Term-frequency baseline ties BM25. These are fixture acceptance criteria, not production relevance SLOs. Five thousand synthetic short facts: index build 47.408 ms and median selective retrieval 0.011 ms in the first full U08 run. No timing threshold or hardware-normalized SLA; DB ingestion, broad-query loads and inference are excluded. Character-based context budget is not a model token-window guarantee.
