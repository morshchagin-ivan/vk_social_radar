...
document.querySelector("#person-detail").innerHTML=`<h2>${esc(p.full_name)}</h2><p><a href="${esc(p.profile_url||"#")}" target="_blank" rel="noreferrer">Открыть профиль VK</a></p><div class="detail-grid"><div class="detail-card"><span>Сообщений</span><strong>${total}</strong></div><div class="detail-card"><span>Активных дней</span><strong>${s.active_days??0}</strong></div><div class="detail-card"><span>Инициатива человека</span><strong>${initiative}%</strong></div></div><h3>Баланс</h3><p>Входящие: <strong>${s.incoming_count??0}</strong> · Исходящие: <strong>${s.outgoing_count??0}</strong></p>${insight}<h3>События</h3><div class="list">${d.events.length?d.events.map(e=>`<div class="list-item"><div><div class="list-title">${esc(eventLabel(e.event_type))}</div><div class="list-meta">${esc(e.details||"")}</div></div><span>${esc(e.event_date)}</span></div>`).join(""):'<div class="empty">Событий нет</div>'}</div>`;document.querySelector("#person-dialog").showModal();}

async function loadMessages(){const rows=await api("/api/messages/leaderboard");document.querySelector("#messages-table-wrap").innerHTML=`<table><thead><tr><th>Человек</th><th>Всего</th><th>Входящие</th><th>Исходящие</th><th>Активные дни</th><th>Инициатива</th><th>Медиана ответа</th></tr></thead><tbody>${rows.map(i=>`<tr><td><strong>${esc(i.full_name)}</strong></td><td>${i.total_messages}</td><td>${i.incoming_count}</td><td>${i.outgoing_count}</td><td>${i.active_days}</td><td>${i.person_initiative_pct}%</td><td>${i.median_reply_minutes??"—"} мин</td></tr>`).join("")}</tbody></table>`;}
...
function renderAIPeople(){document.querySelector("#ai-people").innerHTML=state.people.map(p=>`<article class="person-card"><h3>${esc(p.full_name)}</h3><p>${p.total_messages} сообщений · ${p.active_days} активных дней</p><div class="actions"><span class="badge">${p.is_friend?"Друг":p.is_follower?"Подписчик":"Контакт"}</span><button class="primary ai-run" data-ai-person="${p.id}">Анализировать</button></div><div id="ai-result-${p.id}"></div></article>`).join("");document.querySelectorAll(".ai-run").forEach(b=>b.onclick=()=>runInsight(Number(b.dataset.aiPerson),b));}
...
function renderCollectorPreview(p){
  ...
  document.querySelector("#collector-report").innerHTML=
    ...
    `</div><p><strong>${esc(p.kind)}</strong> · найдено ${p.count} · ${esc(p.collected_at)}</p>`;
  ...
  document.querySelector("#collector-preview").innerHTML=
    ...
    p.items.slice(0,250).map(i=>`<tr><td><strong>${esc(i.full_name)}</strong></td><td>${esc(isDialogs?(i.peer_id??"—"):(i.vk_id??i.screen_name??"—"))}</td><td><a href="${esc(i.dialog_url||i.profile_url||"#")}" target="_blank">Открыть</a></td></tr>`).join("")+
    `</tbody></table>`+
