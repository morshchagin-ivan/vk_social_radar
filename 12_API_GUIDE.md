...
**IMPLEMENTED U04 contract + U06 privacy/access controls — 2026-09-24.** Canonical contract: [11_OPENAPI.yaml](11_OPENAPI.yaml). Source of truth for authoring: FastAPI route metadata in [main.py](app/main.py), [response models](app/api_models.py) and [request/error metadata](app/api_contract.py). The root file is the generated published artifact, not a second hand-maintained definition. [Inventory](docs/certification/U04_RUNTIME_API_INVENTORY.md) · [Evidence](docs/certification/U04_API_CONTRACT_REPORT.md).
...
No Bearer token, API authentication or multi-user authorization is implemented. The fixed loopback launcher plus local Host/same-origin checks enforce the browser boundary; the local OS account remains trusted. Collector `check-auth` checks VK browser login, not the HTTP caller. U06 rejects unexpected Host with 400 and foreign/mismatched Origin or cross-site Fetch Metadata with 403. No permissive CORS is enabled; no-Origin local tools remain supported. Never infer readiness from health or full inference availability from model discovery.
...
| Settings | GET and PUT `/api/settings` | string-valued mapping; PUT accepts object, validates/normalizes base URL to loopback, stringifies model/temperature and ignores other keys |
...
| Browser control | POST `/api/collector/start`, `/close`, `/check-auth`; DELETE `/api/collector/profile`; GET `/api/collector/status` | status or ok/deleted; profile deletion is the existing explicit action; profile paths are replaced by a non-sensitive label; current_url is origin-only and last_error is a safe category or null |
...
Multipart fields: file and import_type required; import_type accepts `relations` or `message_stats` in business validation. Relations additionally require relation_type friend/follower. snapshot_date may be omitted. JSON/CSV/TSV/HTML/HTM/ZIP are supported; at most 100 MiB plus one byte is read, and oversized content is rejected. ZIP declared and actual expansion are capped at 100 MiB, with at most 1000 entries. Safe relative members are parsed in memory without extraction; traversal/absolute members are rejected. No archive-wide transaction is claimed. HTML extraction is UNKNOWN. Result is snapshot fields, imported count, or imported + processed_files, with job_id and stored_as. stored_as is a unique basename, not an absolute path. Upload filenames reject traversal/absolute Windows or POSIX paths; writes use exclusive creation in the checked import root. Multipart framework spooling is not a full transport/CPU DoS guarantee.

Synthetic settings update: `PUT /api/settings` with `{"lmstudio_model":"synthetic-model","lmstudio_temperature":0.2}` stores string values. The three accepted keys are lmstudio_base_url, lmstudio_model, lmstudio_temperature. lmstudio_base_url must be a parsed HTTP(S) loopback string: localhost normalizes to 127.0.0.1, ::1 is supported; public/LAN/userinfo/query/fragment and ambiguous forms are rejected with 400 before any settings write. No remote opt-in exists; proxies and redirects are disabled. Unsafe legacy URLs return empty and block inference without rewriting storage. No retry knobs/provider selector or temperature bounds are added. Settings unknown keys are ignored.
...
| 403 | `{"detail":"Same-origin request required"}` | Origin/Fetch Metadata boundary; not application authentication |
| 404 | `{"detail":"..."}` | missing person, preview or organization job |
| 422 | `{"detail":[{"loc":["path","person_id"],"msg":"...","type":"..."}]}` | Validation with fixed message/type and safe location; input/ctx are omitted |
...
Only applicable mapped statuses are listed per operation. Generic unhandled 500 is documented everywhere. 400 Host and 403 Origin checks are documented for every route. There is no invented bearer 401 policy or conflict 409 mapping. Frontend continues to read JSON detail when present and otherwise falls back to HTTP status text.
